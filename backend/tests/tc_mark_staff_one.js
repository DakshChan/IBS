 ...
 * @returns {`/instructor/course/${string}/mark/one`}
 */
const getStaffMarkOneEndpoint = (course_id) => {
    return `/instructor/course/${course_id}/mark/one`;
};
...
describe('Get Mark as Instructor Endpoint', () => {
    ...
    it('When task field & username is passed in payload, staff should be able to get specific task marks of a student. Hidden marks should also be returned.', (done) => {
        chai.request(BASE_API_URL)
            .get(getStaffMarkOneEndpoint(1))
            ...
            .end((err, res) => {
                ...
                expect(marks).to.not.have.property('cscstudentuserb');
                done();
            });
    });

    it('When username is passed in payload and without task, staff should be able to get specific task marks of a student. Hidden marks should also be returned.', (done) => {
        chai.request(BASE_API_URL)
            .get(getStaffMarkOneEndpoint(1))
            .set('Authorization', cscInstructorToken)
            ...
            .end((err, res) => {
                ...
                expect(res.body.marks).to.not.have.property('cscstudentuserb');
    ...
    it('An instructor who enters the wrong course id, should not be able to get anything', (done) => {
        chai.request(BASE_API_URL)
            .get(getStaffMarkOneEndpoint(100))
            .set('Authorization', cscInstructorToken)
            .query(markListPayload[0])
            .end((err, res) => {
                expect(res).to.have.status(400);
    ...
    it('An instructor who doesnt enter a username, should not be able to get anything', (done) => {
        const badPayload = {
            task: 'Assignment-1',
            total: false
        }
        ...
        chai.request(BASE_API_URL)
            ...
            .end((err, res) => {
                expect(res).to.have.status(400);
                expect(res.body).to.have.property('message', 'The username is missing or has invalid format.');
                done();
            });
    });
