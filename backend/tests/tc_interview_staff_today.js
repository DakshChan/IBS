 ...
 * @returns {`/instructor/course/${string}/interview/today`}
 */
const interviewStaffTodayEndpoint = (course_id) => {
    return `/instructor/course/${course_id}/interview/today`;
};
...
describe('Get a specific students mark as Instructor Endpoint', () => {
    ...
    it('Get specific interview as instructor', (done) => {
        chai.request(BASE_API_URL)
            .get(interviewStaffTodayEndpoint(1))
    ...
    it('Get interviews with incorrect course id', (done) => {
        chai.request(BASE_API_URL)
            .get(interviewStaffTodayEndpoint(100))
            .set('Authorization', cscInstructorToken)
            .query(payload[0])
            .end((err, res) => {
                expect(res).to.have.status(400);
